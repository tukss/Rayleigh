Module Physics
	...
	Type(rmcontainer3D), Allocatable :: ftemp1(:), ftemp2(:)
...
Contains
	...
	Subroutine rlm_spaceb()
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do i = 1,2
				Do r = 1, N_R
					wsp%s2b(mp)%data(m:l_max,r,i,wvar) = wsp%s2b(mp)%data(m:l_max,r,i,wvar)*over_l_l_plus1(m:l_max)
				Enddo
			Enddo
		Enddo
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do i = 1,2
			do r = 1, N_r
				ftemp1(mp)%data(m:l_max,r,i) = ( ftemp2(mp)%data(m:l_max,r,i)- &
					& ftemp1(mp)%data(m:l_max,r,i) )*over_l_l_plus1(m:l_max)
			Enddo
			Enddo
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do r = 1, N_R
				rind1 = rmn1+r
				rind2 = rmn2+r
				wsp%s2b(mp)%data(m:l_max,r,i,pvar) = ( wsp%s2b(mp)%data(m:l_max,r,i,zvar)+ &
					& ftemp2(mp)%data(m:l_max,r,i) )*over_l_l_plus1(m:l_max)
			Enddo
			Enddo
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do i = 1, 2
			Do r = 1, N_R
				wsp%s2b(mp)%data(m:l_max,r,i,zvar) = ftemp1(mp)%data(m:l_max,r,i,zvar)
			Enddo
			Enddo
		...
		Do mp = my_mp%min, my_mp%max
			m = m_values(mp)
			if (m .eq. 0) then
				wsp%s2b(mp)%data(0,:,:,wvar) = 0.0d0				
				wsp%s2b(mp)%data(0,:,:,zvar) = 0.0d0
				wsp%s2b(mp)%data(0,:,:,pvar) = 0.0d0
			endif
		Enddo
	...
	Subroutine Velocity_Components()
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do i = 1, 2
			Do r = 1, N_R
				wsp%s2a(mp)%data(m:l_max,r,i,vr) = l_l_plus1(m:l_max)*wsp%s2a(mp)%data(m:l_max,r,i,vr)*OneOverRSquared(r)
			Enddo
			Enddo
		Enddo
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do i = 1, 2
			do r = 1, N_R		
				wsp%s2a(mp)%data(:,r,i,vtheta) = ftemp1(mp)%data(:,r,i)*OneOverRSquared(r)
			enddo
			Enddo
		Enddo
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do i = 1, 2
			do r = 1, N_R	
				wsp%s2a(mp)%data(:,r,i,vphi) = ftemp1(mp)%data(:,r,i)*OneOverRSquared(r)
			enddo
			Enddo
		Enddo
	...
	Subroutine Velocity_Derivatives()
		...
		Do mp = my_mp%min, my_mp%max
			m = m_values(mp)
			ftemp1(mp)%data(:,:,:) = ftemp1(mp)%data(:,:,:)+ftemp2(mp)%data(:,:,:)-wsp%s2a(mp)%data(:,:,:,vtheta)
			Do i = 1, 2
			do r = 1, N_R			
				wsp%s2a(mp)%data(:,r,i,dvtdr) = ftemp1(mp)%data(:,r,i)*OneOverRSquared(r)
			enddo
			Enddo
		Enddo
		...
		!/////////////////////////////////
		!sinphi dv phi dr 
		Call d_by_dphi(wsp%s2a,d2wdr2,   ftemp1)	   
		Call d_by_dtheta(wsp%s2a,dzdr,	ftemp2)	  		

		Do mp = my_mp%min, my_mp%max
			m = m_values(mp)
			ftemp1(mp)%data(:,:) = ftemp1(mp)%data(:,:,:)-ftemp2(mp)%data(:,:,:)-wsp%s2a(mp)%data(:,:,:,vphi)
			Do i = 1, 2
			do r = 1, N_R	
				wsp%s2a(mp)%data(:,r,i,dvpdr) = ftemp1(mp)%data(:,r,i)*OneOverRSquared(r)
			enddo
			Enddo
		Enddo
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do r = 1, N_R
			Do l = m, l_max
				wsp%s2a(mp)%data(l,r,i,dvrdr) = l_l_plus1(l)*wsp%s2a(mp)%data(l,r,i,dvrdr)*ovrsq_repeated(r)	! real part
			Enddo
			Enddo
			Enddo
			Do r = 1,N_R
				wsp%s2a(mp)%data(:,r,i,dvrdr) = wsp%s2a(mp)%data(:,r,i,dvrdr)-2.0d0*wsp%s2a(mp)%data(:,r,i,vr)*OneOverRSquared(r)
			Enddo
		Enddo
		...
		Do mp = my_mp%min, my_mp%max
			...
			Do i = 1, 2
			Do r = 1, N_R
				wsp%s2a(mp)%data(m:l_max,r,i,zvar) = l_l_plus1(m:l_max)*wsp%s2a(mp)%data(m:l_max,r,i,zvar)*OneOverRSquared(r)
			Enddo
			Enddo
		Enddo
	...
	Subroutine Allocate_rlm_Field(arr)
		...
		Do mp = my_mp%min, my_mp%max
			m = m_values(mp)
			Allocate(arr(mp)%data(m:l_max,1:N_R,1:2))
		Enddo
